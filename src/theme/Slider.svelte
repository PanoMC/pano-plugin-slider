...
<script>
  ...
  function getBoxStyle(settings) {
    ...
    const bg = hexToRgba(
      settings.captionBackground || "#000000",
      settings.captionOpacity || 0.5,
    );
    const blur =
      settings.captionStyle === "glass"
        ? `blur(${settings.blurAmount || 5}px)`
        : "none";
          ...
          <div class="carousel-indicators">
            {#each sliderItems as _, i}
              <button
                ...
                aria-label={$_("components.slider.slide", { index: i + 1 })}
              ></button>
            {/each}
          </div>
        {/if}
        ...
        {#if settings.controls && sliderItems.length > 1}
          ...
          >
            ...
            <span class="visually-hidden"
              >{$_("components.slider.previous")}</span
            >
          </button>
          <button
